...
func interfaceStats(name string, rxBytes, txBytes uint64) stats.InterfaceStats {
	return stats.InterfaceStats{
		Name:    name,
		RxBytes: &rxBytes,
		TxBytes: &txBytes,
	}
}
func networkStats(interfaces []stats.InterfaceStats, ts time.Time) *stats.NetworkStats {
	return &stats.NetworkStats{
		Time:           metav1.Time{ts},
		InterfaceStats: interfaces[0],
		Interfaces:     interfaces,
	}
}

func podStats(namespace, name string, network *stats.NetworkStats, containers ...stats.ContainerStats) stats.PodStats {
...
func verifyNode(nodeName string, summary *stats.Summary, batch *sources.CustomMetricsBatch) {
	var cpuUsage, memoryUsage, nwRxBytes, nwTxBytes resource.Quantity
	var timestamp time.Time
	hasMetrics := false
	expectPoint := sources.CustomMetricsPoint{
	...
	if summary.Node.CPU != nil {
		if summary.Node.CPU.UsageNanoCores != nil {
			...
			hasMetrics = true
			expectPoint.Usage[corev1.ResourceCPU] = cpuUsage
		}
	...
	if summary.Node.Memory != nil {
		if summary.Node.Memory.WorkingSetBytes != nil {
			...
			hasMetrics = true
			expectPoint.Usage[corev1.ResourceMemory] = memoryUsage
		}
		if timestamp.IsZero() {
	...
	if summary.Node.Network != nil {
		hasMetrics = true
		nwTxBytes = *resource.NewQuantity(int64(*summary.Node.Network.InterfaceStats.TxBytes), resource.BinarySI)
		nwRxBytes = *resource.NewQuantity(int64(*summary.Node.Network.InterfaceStats.RxBytes), resource.BinarySI)
		if summary.Node.Network.InterfaceStats.TxBytes == nil && summary.Node.Network.InterfaceStats.RxBytes == nil {
			for _, i := range summary.Node.Network.Interfaces {
				if i.TxBytes != nil {
					nwTxBytes.Add(*resource.NewQuantity(int64(*i.TxBytes), resource.BinarySI))
				}
				if i.RxBytes != nil {
					nwRxBytes.Add(*resource.NewQuantity(int64(*i.RxBytes), resource.BinarySI))
				}
			}
		}
		if timestamp.IsZero() {
			timestamp = summary.Node.Network.Time.Time
		}
		expectPoint.Usage[sources.ResourceNetworkRxBytes] = nwRxBytes
		expectPoint.Usage[sources.ResourceNetworkTxBytes] = nwTxBytes
	}
	expectPoint.Timestamp = timestamp
	objmetrics := batch.Nodes[sources.NullNamespace]

	actuallyPoint, ok := objmetrics[nodeName]
	if hasMetrics {
		Expect(ok).To(BeTrue())
...
func verifyPods(summary *stats.Summary, batch *sources.CustomMetricsBatch) {
	allPods := sources.NamespaceMetrics{}
	for _, pod := range summary.Pods {

		// containers := make([]sources.ContainerMetricsPoint, len(pod.Containers))
		hasMetrics := false
		cpuLostMetrics := false
		memLostMetrics := false
		podPoint := sources.CustomMetricsPoint{
			Usage: sources.ResourceList{},
		}
		var cpuUsageTotal, memoryUsageTotal, nwRxBytes, nwTxBytes resource.Quantity
		...
		for _, container := range pod.Containers {
			...
			if container.CPU != nil && container.CPU.UsageNanoCores != nil {
				...
				if earliest == nil || earliest.After(container.CPU.Time.Time) {
					earliest = &container.CPU.Time.Time
				}
				// timestamp = container.CPU.Time.Time
				cpuUsageTotal.Add(cpuUsage)
			...
			if container.Memory != nil && container.Memory.WorkingSetBytes != nil {
				...
				// }
				if earliest == nil || earliest.After(container.Memory.Time.Time) {
					earliest = &container.Memory.Time.Time
				}
				memoryUsageTotal.Add(memoryUsage)
		...
		if !cpuLostMetrics {
			hasMetrics = true
			podPoint.Usage[corev1.ResourceCPU] = cpuUsageTotal
		}
		if !memLostMetrics {
			hasMetrics = true
			podPoint.Usage[corev1.ResourceMemory] = memoryUsageTotal
		}
		if pod.Network != nil {
			missMetrics := false
			if pod.Network.InterfaceStats.TxBytes == nil && pod.Network.InterfaceStats.RxBytes == nil {
				...
				nwTxBytes = *resource.NewQuantity(int64(*pod.Network.InterfaceStats.TxBytes), resource.BinarySI)
				nwRxBytes = *resource.NewQuantity(int64(*pod.Network.InterfaceStats.RxBytes), resource.BinarySI)
			}
			if earliest == nil || earliest.After(pod.Network.Time.Time) {
				earliest = &pod.Network.Time.Time
			}
			if !missMetrics {
				hasMetrics = true
				podPoint.Usage[sources.ResourceNetworkTxBytes] = nwTxBytes
				podPoint.Usage[sources.ResourceNetworkRxBytes] = nwRxBytes
			}
		}
		if earliest != nil {
			podPoint.Timestamp = *earliest
		}
		if !hasMetrics {
			continue
		}

		singleNSPods, ok := allPods[pod.PodRef.Namespace]
		if !ok {
			singleNSPods = sources.ObjectMetrics{}
			allPods[pod.PodRef.Namespace] = singleNSPods
		}
		singleNSPods[pod.PodRef.Name] = &podPoint
	}
	for namespace, singleNSPods := range batch.Pods {
		for name, actuallyPoint := range singleNSPods {
			expectnspods, ok := allPods[namespace]
			Expect(ok).To(BeTrue())
			expectPoint, ok := expectnspods[name]
			Expect(ok).To(BeTrue())
			// err := equalCustomMetricsPoint(actuallyPoint, expectPoint)
	...
	for namespace, singleNSPods := range allPods {
...
var _ = Describe("Summary Source", func() {
				...
				Pods: []stats.PodStats{
					...
					podStats("ns3", "pod1",
						networkStats(
							...
							scrapeTime.Add(400*time.Millisecond)),
	...
	It("should use the scrape time from the CPU, falling back to memory if missing", func() {
		...
		client.metrics.Pods[1].Containers[0].CPU.Time = metav1.Time{}
		...
		Expect(batch.Nodes[sources.NullNamespace]["node1"].Timestamp).To(Equal(client.metrics.Node.Memory.Time.Time))
		Expect(batch.Pods["ns1"]["pod2"].Timestamp).To(Equal(client.metrics.Pods[1].Containers[0].Memory.Time.Time))
		Expect(batch.Pods["ns1"]["pod1"].Timestamp).To(Equal(client.metrics.Pods[0].Containers[0].CPU.Time.Time))
	})
	...
	It("should handle larger-than-int64 CPU or memory values gracefully", func() {
		...
		client.metrics.Pods[0].Containers[0].CPU.UsageNanoCores = &minusTen
		client.metrics.Pods[1].Containers[0].Memory.WorkingSetBytes = &minusOneHundred
		client.metrics.Pods[2].Network.InterfaceStats.RxBytes = &minusOneHundred
		client.metrics.Pods[2].Network.Interfaces = []stats.InterfaceStats{}
		...
		podRxBytes := *resource.NewScaledQuantity(int64(minusOneHundred/10), 1)
		podRxBytes.Format = resource.BinarySI
		Expect(*batch.Nodes[sources.NullNamespace]["node1"].Usage.Memory()).To(Equal(nodeMem))
		Expect(*batch.Nodes[sources.NullNamespace]["node1"].Usage.Cpu()).To(Equal(*resource.NewScaledQuantity(int64(plusTwenty/10), -8)))
		Expect(*batch.Pods["ns1"]["pod1"].Usage.Cpu()).To(Equal(*resource.NewScaledQuantity(int64(minusTen/10+minusTen/10), -8)))
		Expect(*batch.Pods["ns1"]["pod2"].Usage.Memory()).To(Equal(podMem))
		Expect(*batch.Pods["ns2"]["pod1"].Usage.NetworkRxBytes()).To(Equal(podRxBytes))
	})
	It("should get pod metrics but ignore node metrics when node data miss", func() {
		By("removing some Node data from the raw summary")
		client.metrics.Node.CPU = nil
		client.metrics.Node.Network = nil
		...
		By("removing all node data from the raw summary")
		client.metrics.Node.Memory = nil
		client.metrics.Node.CPU = nil
		client.metrics.Node.Network = nil
	...
	It("should get pod metrics but ignore pod metrics when pod data miss", func() {
		By("removing some pod data from the raw summary")
		client.metrics.Pods[0].Containers[1].CPU = nil
		client.metrics.Pods[0].Containers[0].CPU = nil
		By("collecting the batch")
		batch, err := src.Collect(context.Background())
		Expect(err).To(HaveOccurred())
		...
		client.metrics.Pods[0].Containers[1].CPU = nil
		client.metrics.Pods[0].Containers[0].CPU = nil
		client.metrics.Pods[0].Containers[0].Memory = nil
		client.metrics.Pods[0].Containers[1].Memory = nil
		client.metrics.Pods[0].Network = nil
		By("collecting the batch")
		batch, err = src.Collect(context.Background())
		Expect(err).To(HaveOccurred())
	...
	It("should get pod network when the  default interface is empty", func() {
		By("removing some pod data from the raw summary")
		client.metrics.Pods[0].Network.InterfaceStats.RxBytes = nil
		client.metrics.Pods[0].Network.InterfaceStats.TxBytes = nil
	...
	It("should get pod network when the  network interface is empty", func() {
		By("removing some pod data from the raw summary")
		client.metrics.Pods[0].Network.InterfaceStats.RxBytes = nil
		client.metrics.Pods[0].Network.InterfaceStats.TxBytes = nil
		client.metrics.Pods[0].Network.Interfaces[0].RxBytes = nil
		client.metrics.Pods[0].Network.Interfaces[0].TxBytes = nil
		client.metrics.Pods[0].Network.Interfaces[1].RxBytes = nil
		client.metrics.Pods[0].Network.Interfaces[1].TxBytes = nil
...
func nodeNames(nodeList []*corev1.Node, addrs []string) []string {
	var res []string
	for i, node := range nodeList {
		res = append(res, NewSummaryMetricsSource(nodes.Info{ConnectAddress: addrs[i], Name: node.Name}, nil).Name())
	}
	return res
}
...
var _ = Describe("Summary Source Provider", func() {
	...
	BeforeEach(func() {
		...
		fakeClient = &fakeKubeletClient{}
		addrResolver := nodes.NewPriorityAddressResolver(nodes.DefaultAddressTypePriority)
	...
	Describe("when choosing node addresses", func() {
		...
		It("should prefer addresses according to the order of the types first", func() {
			...
			nodeLister.nodes[0].Status.Addresses = []corev1.NodeAddress{
				{Type: nodes.DefaultAddressTypePriority[3], Address: "skip-val1"},
				{Type: nodes.DefaultAddressTypePriority[2], Address: "skip-val2"},
				{Type: nodes.DefaultAddressTypePriority[1], Address: "correct-val"},
			}
		...
		It("should prefer the first address that matches within a given type", func() {
			By("setting the first node to have multiple addresses and setting all nodes to ready")
			nodeLister.nodes[0].Status.Addresses = []corev1.NodeAddress{
				{Type: nodes.DefaultAddressTypePriority[1], Address: "skip-val1"},
				{Type: nodes.DefaultAddressTypePriority[0], Address: "correct-val"},
				{Type: nodes.DefaultAddressTypePriority[1], Address: "skip-val2"},
				{Type: nodes.DefaultAddressTypePriority[0], Address: "second-val"},
			}
			for _, node := range nodeLister.nodes {
				node.Status.Conditions = []corev1.NodeCondition{
					{Type: corev1.NodeReady, Status: corev1.ConditionTrue},
